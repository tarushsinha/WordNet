...
class StudentTests < Minitest::Test
    ...
    def test_public_commandline
        ...
        assert_equal({:recognized_command => :find, :result => [3]},
                    parser.parse("find e"))
        assert_equal({:recognized_command => :findmany, :result => {"b" => [1], "e" => [3]}},
                    parser.parse("findmany b,e"))
    end
end
